...
class ShopTarget extends THREE.Object3D {
	...
	setupTarget() {
		...
		const geom = new THREE.PlaneGeometry(TARGET_RADIUS * 2, TARGET_RADIUS * 2);
	...
	setupHitArea() {
		const geom = new THREE.SphereGeometry(TARGET_HITAREA_RADIUS, 20, 20);
		const material = new THREE.MeshStandardMaterial({
			color: 0x00ff00,
			metalness: 0,
			roughness: 0,
	...
	onFocus() {
		...
		this.isFocused = true;
		this.targetScale = TARGET_FOCUS_SCALE;
	...
	update(delta) {
		if (this.isFocused) {
			...
			const scalar = window.mobile ? 1 : 1 - (this.restToTargetVector.length() / TARGET_MAX_WANDER);
			this.restToTargetVector
				.multiplyScalar(Math.max(0, scalar))
				.clampLength(0, TARGET_MAX_WANDER);
